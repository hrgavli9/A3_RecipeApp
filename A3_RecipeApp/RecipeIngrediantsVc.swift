...
class RecipeIngrediantsVc: UIViewController {
    ...
    private let Ingrediants:UILabel = {
        let lb=UILabel()
        lb.text="""
    ...
    @objc func btnfunc() {
        navigationController?.pushViewController(RecipeInstructionsVc(), animated: true)
    }
    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = .systemYellow
        view.addSubview(Ingrediants)
    ...
    override func viewDidLayoutSubviews() {
        ...
        Ingrediants.frame = CGRect(x: 10, y: Header.bottom+10, width: view.width-20, height: 500)
        myButton.frame = CGRect(x: 300, y: Ingrediants.bottom+10, width:view.width-50, height:40)
    }
}
